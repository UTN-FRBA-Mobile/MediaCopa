...
fun DireccionesScreen(navController: NavController) { // hay que comentar los parametros para poder usar el preview
    ...
    Column(
        ...
        var todoListState = remember { mutableStateListOf("", "") }

        repeat(todoListState.size) { index ->
            Row() {
                TextField(
                    label = { Text("Direccion") },
                    value = todoListState[index],
                    onValueChange = { todoListState[index] = it },
                    modifier = Modifier.padding(20.dp)
                )
                Button(
                    onClick = {
                        if (todoListState.size > 2) {
                            todoListState.removeAt(index)
                        }
                    }
                ) {
                    Text(text = "X")
